...
<form action="<?php echo base_url()?>index.php/proses_db/insert_bank" method="POST">
    <div class="row" id="tambah_data">
    <div class="col-md-4">
      <h3>FORMS TAMBAH BANK SAMPAH</h3>
      <input type="hidden" name="id_bank_sampah" id="id_bank_sampah_edit">
      <input type="hidden" name="id_lokasi" id="id_lokasi_edit">
       <div class="form-group">
        <label for="nama_pengepulan">Nama Bank Sampah</label>
        <input type="text" class="form-control" id="nama_pengepulan" aria-describedby="emailHelp" placeholder="Nama Bank Sampah" name="nama_bank" required="true">
      </div>
      <div class="form-group">
        <label for="nama_penanggung">Nama Penanggung Jawab</label>
        <input type="text" class="form-control" id="nama_penanggung" aria-describedby="emailHelp" placeholder="Nama Penanggung Jawab" name="nama_penanggung" required="true">
      </div>
       <div class="form-group">
        <label for="jumlah_nasabah">Jumlah Nasabah</label>
        <input type="text" class="form-control" id="jumlah_nasabah" placeholder="Jumlah Nasabah" name="jumlah_nasabah" required="true" onkeypress="return validCheck(event)">
      </div>
      <div class="form-group">
  ...
  <div class="col-md-12 col-xs-12 col-sm-12 table-responsive" >
    <table class="table" id="my-table">
      ...
      <tbody>
        <?php 
          $no = 1;
          foreach ($bank_sampah->result() as $row) { 
        ?>
        <tr>
          <td><?php echo $no;?></td>
          ...
          <td>
            <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#Edit_modal" onclick='edit(<?php echo '"'.$row->id_bank_sampah.'"';?>)'><i class="glyphicon glyphicon-pencil"></i></button>
            <button class="btn btn-sm btn-danger" data-toggle="modal" data-target="#delete_modal" onclick='modal_hapus(<?php echo '"'.$row->id_bank_sampah.'"';?>,<?php echo '"'.$row->id_lokasi.'"';?>)'><i class="glyphicon glyphicon-trash"></i></button>
          </td>
        </tr>  
        <?php $no++;} ?>
      </tbody>
...
<div class="modal fade" id="delete_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-confirm">
    <div class="modal-content">
      ...
      <input type="hidden" name="id_bank_sampah" id="id_bank_sampah">
 ...
 <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyCiDdGyp6n2hKHPECuB6JZIT-8dVHCpwI0&language=id&region=ID&callback=initialize&libraries=places"
    ...
    <script type="text/javascript">
      ...
      function modal_hapus(id_bank_sampah,id_lokasi){
        $('#id_bank_sampah').val(id_bank_sampah);
        $('#id_lokasi').val(id_lokasi);
      }
      function hapus(){
        var id_bank_sampah=document.getElementById('id_bank_sampah').value;
        var id_lokasi=document.getElementById('id_lokasi').value;
        window.location.href="<?php echo base_url()?>/index.php/proses_db/hapus_bs?kode="+id_bank_sampah+"&&id_lokasi="+id_lokasi;
      }
      ...
      function edit(id_bank_sampah){
        var xmlhttp=new XMLHttpRequest();
        var param="id_bank_sampah="+id_bank_sampah;
        //alert(id_bank_sampah);
        <?php 
          if($this->session->userdata('level')!='admin'){
            echo 'xmlhttp.open("GET", "'.base_url().'/index.php/proses_db/edit_ajax_bs_kelurahan?"+param, true);';
          }else{
            echo 'xmlhttp.open("GET", "'.base_url().'/index.php/proses_db/edit_ajax_bs_kelurahan?"+param, true);';
          }
        ?>
        xmlhttp.onreadystatechange=function(){
          if(this.readyState == 4 && this.status == 200){
              ...
              $('#nama_pengepulan').val(myObj[0]['nama_bank']);
              $('#nama_penanggung').val(myObj[0]['penanggung_jawab']);
              $('#jumlah_nasabah').val(myObj[0]['jumlah_nasabah']);
              ...
              $('#id_bank_sampah_edit').val(myObj[0]['id_bank_sampah']);
